...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def forward(self,
        ...
        for i in range(len(images)):
            ...
            # if images_tmp is None:
            #     images_tmp = image.unsqueeze(0)
            # else:
            #     images_tmp = torch.vstack((images_tmp, image.unsqueeze(0)))
    ...
    def _onnx_batch_images(self, images, size_divisible=32):
        ...
        for i in range(images[0].dim()):
            max_size_i = torch.max(torch.stack([torch.as_tensor(img.shape[i]) for img in images]).to(torch.float32)).to(torch.int64)
