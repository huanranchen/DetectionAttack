...
```
We provide the generated(detected) INRIAPerson labels. The labels can be downloaded from:
...
* **BaiduCloud** https://pan.baidu.com/s/1tJh-E_0KepziQjsNa8KIJA?pwd=dm85
  * Currently only Yolov2~Yolov5 in Darknet, FasterRCNN & SSD in PyTorch are supported.

### Run
#### Evaluation
The evaluation utilize **Mean Average Precision(mAP)** as the metrics to evaluate the adversarial robustness of the given model and the adversarial patch.
```bash
# fast eval by running .sh
bash test.sh 2 exp3/rerase/v2/v2-aug-fix-1 exp3/rerase/v2/ "fix/coco80 fix/coco91" "Test"
...
#### Training
```bash
CUDA_VISIBLE_DEVICES=0 python train_optim.py -cfg=v3.yaml -s=./results/test -n=test >./results/test.log
```
